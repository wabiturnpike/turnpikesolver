...
## Use
You only need two things to use the solver: a distance set and a rough estimate of the distance set's uncertainty. The estimate only improves runtime, so if you don't have one, just set it at a sufficiently high threshold. To solve an instance,
