...
for tf in test_files:
    ...
    if n > 500:
        continue
    z = open(tf, 'r').readlines()[0].strip().split(' ')
    z = torch.tensor([int(p) for p in z], dtype=torch.float64).view(-1, 1).to(device)
    ...
    for eps in noise:
        ...
        z_best, _ = solver.shift_points(z, solver.solve(z, d_eps, iter=20, mode='patient'))
        runtime = time.time() - start_time
        res[exp_title] = {'time': runtime, 'est': z_best, 'd_noise': d_noise}
        torch.save(res, './pdtests_estimates/res.pt')